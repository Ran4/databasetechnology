...
class DBContext:
    ...
    Each function gathers the minimal amount of information required
...
# Here we define a member function that we can later call repeatidly
    ...
    def get_int(self):
        ...
        If the user fails to submit an integer, it will
        reprompt until an integer is submitted."""
        while True:
    ...
    def select(self):
        ...
        chosenTable = raw_input("Choose table(s): ").split(",")
        tables = [x.strip() + " natural join " for x in chosenTable]
